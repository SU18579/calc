...
{
    //Checking for retarded input
	if (!pointsPerUnit) return;
    //if the axes are off the screen, stop
    ...
    //number of pixels for hashmark conversion
    ...
    //check to make sure there are hash marks on screen
	...
	int started = NO;
	int stillGoing = YES;
    //Draw a pair of vertical and horzontal hash marks each iteration
